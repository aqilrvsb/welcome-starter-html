...
serve(async (req) => {
  ...
  try {
    ...
    // Get user's FreeSWITCH/AlienVOIP configuration
    ...
    if (phoneError || !phoneConfig) {
      throw new Error('Phone configuration not found. Please configure your FreeSWITCH and AlienVOIP settings in Phone Config.');
    }

    // Validate FreeSWITCH/AlienVOIP configuration
    if (!phoneConfig.freeswitch_url || !phoneConfig.sip_username || !phoneConfig.sip_password) {
      throw new Error('FreeSWITCH/AlienVOIP configuration incomplete. Please configure your SIP trunk settings in Phone Config.');
    }

    console.log(`📞 Using FreeSWITCH server: ${phoneConfig.freeswitch_url}`);
    ...
    const callPromises = validPhones.map(async (phoneNumber) => {
      ...
      try {
        ...
        // This will:
        // 1. Call customer via AlienVOIP SIP trunk
