...
serve(async (req) => {
  ...
  try {
    ...
    // Get AI call handler WebSocket URL (Azure STT version)
    // Use production URL directly to avoid localhost issues
    const SUPABASE_URL = Deno.env.get('SUPABASE_URL') || 'https://ahexnoaazbveiyhplfrc.supabase.co';
    const AI_CALL_HANDLER_URL = `${SUPABASE_URL}/functions/v1/ai-call-handler-azure`.replace('https://', 'wss://');
