 ...
 * Uses credits_balance (RM) and converts to minutes at RM 0.15/min
 */
async function validateBalance(userId: string, estimatedMinutes: number) {
  ...
  const accountType = user?.account_type || 'trial';
  const creditsBalanceRM = user?.credits_balance || 0;

  // Convert RM to minutes (RM 0.15 per minute)
  const PRICE_PER_MINUTE = 0.15;
  const balanceMinutes = creditsBalanceRM / PRICE_PER_MINUTE;

  if (creditsBalanceRM <= 0) {
    throw new Error('Insufficient credits: Balance is RM 0.00. Please top up credits.');
  }

  if (balanceMinutes < estimatedMinutes) {
    throw new Error(
      `Insufficient credits: You have RM ${creditsBalanceRM.toFixed(2)} (${balanceMinutes.toFixed(1)} min) but need ~${estimatedMinutes} min (~RM ${(estimatedMinutes * PRICE_PER_MINUTE).toFixed(2)}). Please top up.`
    );
  }

  console.log(`✅ Credits balance: RM ${creditsBalanceRM.toFixed(2)} (${balanceMinutes.toFixed(1)} min remaining)`);
  return { accountType, balanceMinutes };
}

/**
 * Deduct credits after call completes
 * Deducts from credits_balance (RM) at RM 0.15 per minute
 */
async function deductCreditsAfterCall(userId: string, callDurationMinutes: number) {
  ...
  // Convert minutes to RM (RM 0.15 per minute)
  const PRICE_PER_MINUTE = 0.15;
  const costRM = callDurationMinutes * PRICE_PER_MINUTE;

  const currentBalanceRM = user.credits_balance || 0;
  const newBalanceRM = Math.max(0, currentBalanceRM - costRM); // Don't go negative
  const newTotalUsed = (user.total_minutes_used || 0) + callDurationMinutes;

  console.log(`💳 Deducting ${callDurationMinutes.toFixed(2)} min (RM ${costRM.toFixed(2)}) from RM ${currentBalanceRM.toFixed(2)}`);

  const { error: updateError } = await supabaseAdmin
    .from('users')
    .update({
      credits_balance: newBalanceRM,
  ...
  if (updateError) {
    ...
    const remainingMinutes = newBalanceRM / PRICE_PER_MINUTE;
    console.log(`✅ Balance updated: RM ${newBalanceRM.toFixed(2)} (${remainingMinutes.toFixed(1)} min remaining)`);
  }
}
...
async function handleBatchCall(req: Request): Promise<Response> {
  try {
    ...
    console.log(`   ├─ SIP Username: ${sipConfig.sip_username}`);
    console.log(`   ├─ SIP Proxy: ${sipConfig.sip_proxy_primary}`);
    console.log(`   └─ Caller ID: ${sipConfig.sip_caller_id}`);
    console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
...
async function originateCallWithAudioStream(params: any): Promise<string> {
  ...
  // 🎯 DYNAMIC SIP ROUTING: Choose routing method based on account type
  let originateCmd: string;

  // Check if we have Pro user credentials (indicated by presence of sip_username different from Trial)
  const trialUsername = Deno.env.get('TRIAL_SIP_USERNAME') || '646006395';
  const isProUser = sipConfig.sip_username && sipConfig.sip_username !== trialUsername;

  if (isProUser) {
    // ✅ PRO USER: Use dynamic SIP routing via sofia/external with authentication
    console.log(`🔐 PRO USER ROUTING:`);
    console.log(`   ├─ SIP Username: ${sipConfig.sip_username}`);
    console.log(`   ├─ SIP Proxy: ${sipConfig.sip_proxy_primary}`);
    console.log(`   ├─ Caller ID: ${sipConfig.sip_caller_id}`);
    console.log(`   └─ Method: sofia/external with authentication`);

    const authVars = `${vars},sip_auth_username=${sipConfig.sip_username},sip_auth_password=${sipConfig.sip_password}`;
    originateCmd = `api originate {${authVars}}sofia/external/${phoneNumber}@${sipConfig.sip_proxy_primary} &park()`;
  } else {
    // ✅ TRIAL USER: Use static AlienVOIP gateway (shared trunk)
    // Gateway UUID from FreeSWITCH server 178.128.57.106
    const gatewayName = '1360d030-6e0c-4617-83e0-8d80969010cf';
    console.log(`🆓 TRIAL USER ROUTING:`);
    console.log(`   ├─ Gateway: ${gatewayName} (AlienVOIP)`);
    console.log(`   ├─ SIP Proxy: ${sipConfig.sip_proxy_primary}`);
    console.log(`   ├─ Caller ID: ${sipConfig.sip_caller_id}`);
    console.log(`   └─ Method: sofia/gateway/${gatewayName}`);

    originateCmd = `api originate {${vars}}sofia/gateway/${gatewayName}/${phoneNumber} &park()`;
  }

  console.log(`📞 ORIGINATE COMMAND: ${originateCmd}`);
  ...
  // ✅ VERIFY ROUTING: Log success and confirm which method was used
  if (response.includes('OK')) {
    if (isProUser) {
      console.log(`✅ PRO call initiated via sofia/external/${phoneNumber}@${sipConfig.sip_proxy_primary}`);
    } else {
      console.log(`✅ TRIAL call initiated via sofia/gateway/1360d030-6e0c-4617-83e0-8d80969010cf/${phoneNumber}`);
    }
  }
