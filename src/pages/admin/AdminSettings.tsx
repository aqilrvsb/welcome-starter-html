...
export default function AdminSettings() {
  ...
  const [proMaxConcurrentCalls, setProMaxConcurrentCalls] = useState('10');
      ...
      data?.forEach((setting: SystemSetting) => {
        switch (setting.setting_key) {
          ...
          case 'max_concurrent_calls':
            ...
            break;
          case 'pro_max_concurrent_calls':
            setProMaxConcurrentCalls(setting.setting_value);
            break;
  ...
  const handleSave = async () => {
    try {
      ...
      const updates = [
        ...
        { key: 'pro_max_concurrent_calls', value: proMaxConcurrentCalls },
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure limits for Pro account users
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="pro-concurrent">Max Concurrent Calls</Label>
            <Input
              id="pro-concurrent"
              type="number"
              min="1"
              max="50"
              value={proMaxConcurrentCalls}
              onChange={(e) => setProMaxConcurrentCalls(e.target.value)}
              placeholder="10"
            />
            <p className="text-xs text-muted-foreground">
              Maximum number of simultaneous calls per Pro user (1-50). This limit is strictly enforced for batch calls and campaigns.
            </p>
          </div>
        </CardContent>
      </Card>
