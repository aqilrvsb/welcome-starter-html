...
export function useBatchCall(options: UseBatchCallOptions = {}) {
  ...
  const batchCallMutation = useMutation({
    mutationFn: async (data: BatchCallFormData) => {
      ...
      // Check concurrent call limits based on account type
      const { data: settingsData } = await supabase
        .from('system_settings')
        .select('setting_key, setting_value')
        .in('setting_key', ['max_concurrent_calls', 'pro_max_concurrent_calls']);

      let maxConcurrentCalls = 3; // Default
      if (accountType === 'pro') {
        const proSetting = settingsData?.find(s => s.setting_key === 'pro_max_concurrent_calls');
        maxConcurrentCalls = proSetting ? parseInt(proSetting.setting_value) : 10;
      } else {
        const trialSetting = settingsData?.find(s => s.setting_key === 'max_concurrent_calls');
        maxConcurrentCalls = trialSetting ? parseInt(trialSetting.setting_value) : 3;
      }

      // Enforce concurrent call limit
      if (validNumbers.length > maxConcurrentCalls) {
        throw new Error(`You can only make ${maxConcurrentCalls} concurrent calls at once with your ${accountType === 'pro' ? 'Pro' : 'Trial'} account. You tried to call ${validNumbers.length} numbers. Please reduce the number of contacts.`);
      }
